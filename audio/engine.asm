...
; load music
	...
	; pre-emptively disable all channels. for some reason, this does not happen by default.
	xor a
...
ApplyFreqTempoModifiers:
; only in battle
	ld a, [wIsInBattle]
	and a
	ret z
; only "battle sounds" as designated in RB audio engine 2 source
	ld a, [wCurSFX]
	cp SFX_PECK
	ret c
	...
	xor a
	ld [hl], a
; no tempo modifier for noise channel
	ld a, [wCurChannel]
	and $3
	cp $3
	ret z
; if not noise, apply tempo modifier
	ld hl, CHANNEL_TEMPO
	add hl, bc
	ld a, [wTempoModifier]
	add $80
